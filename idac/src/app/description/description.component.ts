...
export class DescriptionComponent implements OnInit {
  ...
  describe(tag) {
    ...
    /*
      ...
      ret += `범위 ${tag.min}부터 ${
        tag.max ? tag.max
          : this.info.children[4].children
            .map(bargroup => bargroup.children.map(bar => bar.value))
            .reduce((a, b) => a.concat(b)).reduce((a, b) => Math.max(a, b)) }.`;
      if (tag.lines.length) {
      ...
      const basic_description = `${tag.item}. `;
      if (tag._annotation) {
        tag._annotation.components.forEach(component => {
          if (component.visible) {
            if (component.type === 'trendline') {
              ret += `이 범례에 추세선이 그려져 있습니다. `;
              const series = +tag._annotation.target.items[0].elements.slice(1) - 2;
              const data = this.info.children[4].children.map(bargroup => {
                return bargroup.children[series].value;
              });
              const increase = Math.round(10 * (data[data.length - 1] - data[0])) / 10;
              if (increase > 0) {
                ret += `전체 기간 동안 ${increase}만큼 상승했습니다. `;
              } else {
                ret += `전체 기간 동안 ${-increase}만큼 하락했습니다. `;
              }
            }
          }
        });
      }
      if (this.keyboardEventName.endsWith('Annotation')) {
        ret = ret + basic_description;
      } else {
        ret = basic_description + ret;
      }
      ...
      const bargroup = this.getElement(tag.parentId);
      const marks = this.getElement(bargroup.parentId);
      const graph = this.getElement(marks.parentId);
      const y = graph.children[1];
      if (['moveToNextDataPoint', 'moveToPreviousDataPoint'].indexOf(this.keyboardEventName) >= 0) {
        ...
        const basic_description = `막대 그룹이름 ${bargroup.name} 범례 ${tag.key} ${y.label} ${tag.value}. `;
        const annotation_description = tag.highlighted ? `강조되어 있는 막대입니다. ` : '';

        if (this.keyboardEventName.endsWith('Annotation')) {
          ret += annotation_description + basic_description;
        } else {
          ret += basic_description + annotation_description;
        }
      }
